    ...
    <div class="card shadow-sm border-0">
      <div class="card-body text-center p-5">
            ...
            <ul class="list-group list-group-flush">
              ...
              <li class="list-group-item d-flex justify-content-between align-items-center">
                <span>
                  ...
                  Referral Code: <span id="refCode"><?= !empty($user['ref_code']) ? htmlspecialchars($user['ref_code']) : '-' ?></span>
                </span>

                <!-- Tombol Copy -->
                <button class="btn btn-sm btn-outline-secondary" onclick="copyReferral()" title="Copy">
        ...
        <div class="d-flex justify-content-center gap-3">
          <a href="<?= site_url('dashboard/profile/edit'); ?>" class="btn btn-primary">
...
function copyReferral() {
  let text = document.getElementById("refCode").innerText;
  navigator.clipboard.writeText(text).then(() => {
    // pakai alert sederhana
    alert("Kode berhasil dicopy: " + text);

    // atau kalau mau lebih keren, ganti icon jadi check sebentar
    let btn = event.currentTarget.querySelector("i");
    btn.classList.replace("fa-clipboard", "fa-check");
    setTimeout(() => btn.classList.replace("fa-check", "fa-clipboard"), 2000);
  });
}
</script>
